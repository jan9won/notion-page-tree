import { buildPageTreeToDepth } from '../buildPageTreeToDepth';
import collection from './mockData/collection.json';
import { FlatEntity } from '../../../types';

describe('buildPageTreeToDepth', () => {
	//
	it('Finds and returns subtree to given depth.', () => {
		//
		const subTree = buildPageTreeToDepth(
		...
		expect(subTree).toStrictEqual({
			...
			blockContentPlainText: ' content body   ',
			children: [
				{
					...
					children: [
						{
							...
							blockContentPlainText: ' More More Nested Page   ',
